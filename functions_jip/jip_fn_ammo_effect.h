...
{
	TESAmmo *ammo;
	if (ExtractArgs(EXTRACT_ARGS, &ammo) && IS_TYPE(ammo, TESAmmo))
...
bool Cmd_GetNthAmmoEffect_Execute(COMMAND_ARGS)
{
	*result = 0;
	TESAmmo *ammo;
	UInt32 idx;
	if (ExtractArgs(EXTRACT_ARGS, &ammo, &idx) && IS_TYPE(ammo, TESAmmo))
	{
...
bool Cmd_RemoveAmmoEffect_Execute(COMMAND_ARGS)
{
	TESAmmo *ammo;
	TESAmmoEffect *effect;
	if (ExtractArgs(EXTRACT_ARGS, &ammo, &effect) && IS_TYPE(ammo, TESAmmo) && IS_TYPE(effect, TESAmmoEffect) && ammo->effectList.Remove(effect))
		*result = 1;
	else *result = 0;
	return true;
}
...
bool Cmd_GetAmmoEffectTraitNumeric_Execute(COMMAND_ARGS)
{
	*result = 0;
	TESAmmoEffect *effect;
	UInt32 traitID;
	if (!ExtractArgs(EXTRACT_ARGS, &effect, &traitID) || NOT_TYPE(effect, TESAmmoEffect)) return true;
	switch (traitID)
	{
	case 0:
...
bool Cmd_SetAmmoEffectTraitNumeric_Execute(COMMAND_ARGS)
{
	...
	UInt32 traitID;
	float fVal;
	if (!ExtractArgs(EXTRACT_ARGS, &effect, &traitID, &fVal) || NOT_TYPE(effect, TESAmmoEffect) || (fVal < 0)) return true;
