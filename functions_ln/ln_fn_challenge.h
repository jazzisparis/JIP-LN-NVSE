...
bool Cmd_GetChallengeForm1_Execute(COMMAND_ARGS)
{
	*result = 0;
	TESChallenge *challenge;
	if (ExtractArgs(EXTRACT_ARGS, &challenge) && IS_TYPE(challenge, TESChallenge) && challenge->SNAM)
...
bool Cmd_GetChallengeForm2_Execute(COMMAND_ARGS)
{
	*result = 0;
	TESChallenge *challenge;
	if (ExtractArgs(EXTRACT_ARGS, &challenge) && IS_TYPE(challenge, TESChallenge) && challenge->XNAM)
