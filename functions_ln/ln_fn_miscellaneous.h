...
DEFINE_COMMAND_ALT_PLUGIN_EXP(ar_Cat, pinto_Cat, 0, 3, kNVSEParams_TwoArrays_OneOptionalBool);
...
bool Cmd_ar_Cat_Execute(COMMAND_ARGS)
{
	*result = 0;	//bSuccess
		...
		eval.ExtractArgs())
	{
		...
		NVSEArrayVar* catArray = eval.GetNthArg(1)->GetArrayVar();
		if (!inArray || !catArray)
			return true;
		bool overrideOldKeys = false;
		if (eval.NumArgs() == 3)
			overrideOldKeys = eval.GetNthArg(2)->GetBool();
		int const type = GetContainerType(inArray);
		if (type != GetContainerType(catArray))
			return true;
		bool const isPacked = type == NVSEArrayVarInterface::kArrType_Array;
